<script lang="ts">
  ...
  try {
    ...
    // Remove subscriptionNo before update so ARRAYFORMULA remains untouched
    const { subscriptionNo, ...rowWithoutSubscriptionNo } = currentRow;

    // Update SUBSCRIPTION — do NOT write to subscriptionNo column
    await postSheet({
      action: "update",
      rows: [
        {
          ...rowWithoutSubscriptionNo,
    ...
    // Insert APPROVAL log — no approvalNo (generated by Google Sheet)
    await postSheet({
      action: "insert",
      rows: [
        {
          ...
          subscriptionNo: dialogState.selectedRow.subscriptionNo, // reference is OK
